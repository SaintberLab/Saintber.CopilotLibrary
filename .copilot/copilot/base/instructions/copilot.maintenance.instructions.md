...
- Store finalized Traditional Chinese outputs under `.copilot/<module>/composed/` using corresponding relative paths.
...
- Every `.github/` file update MUST be accompanied by a corresponding update to the matching `.copilot/<module>/composed/` file with the complete Traditional Chinese translation in the same operation. Composed file updates are mandatory and must not be skipped or deferred.
