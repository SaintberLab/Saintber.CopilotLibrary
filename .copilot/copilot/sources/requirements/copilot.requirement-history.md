...
### 2026-04-07 11 - module-based-maintain-workflow-restructure

- Recorded At: `2026-04-07`
- Change Summary: 將 `copilot.maintain` 主流程改為 module 化 authoring 架構：需求歷程預設改存 `.copilot/<module>/sources/requirements/`，維護更新改寫入 `.copilot/<module>/base/` 與 `.copilot/<module>/composed/`；同時移除 `.github/TOOLS.md` 依賴，改為 module README；CLI 調整為可解析 `templates/<module>/{agents|instructions|prompts|skills}` 佈局並維持對舊模板路徑的相容。
- Affected Artifacts: `copilot.maintenance.instructions.md`, `copilot.maintainer.agent.md`, `copilot.maintain.prompt.md`, `.copilot/README.md`, `.copilot/<module>/README.md`, `templates/<module>/README.md`, `src/cli.js`, `README.md`, `CHANGELOG.md`, `copilot.requirement-history.md`

```md
請再優化 copilot-maintain 主流程

- .copilot / templates 目錄改為針對不同 module 建立不同目錄的設計 如 kb 系列就會建立 /.copilot/kb 與 /templates/kb 存放相關 agents/skills/prompts/instructions
- .copilot、.github 目錄維持 maintain 時立即更新，templates 於使用者宣告 release 時才更新
- .github 目錄不再產生 TOOLS.md，.copilot/composed/TOOLS.md 與 /templates/TOOLS.md 則拆分至各 modules 目錄下的 README.md，
如 .copilot/kb/README.md
- 同步修改 /src/cli.js，使其可以對齊新的目錄結構，調整完成需進行測試
- 製作層結構如 .copilot/README.md，soruces/base/composed 目錄路徑請修正為 /模組/sources, /模組/base, /模組/composed；原目錄下的內容也請依模組搬遷至新目錄
- 調整 instructions 設計流程
  - 原始的 prompt 來源應該是 /.copilot/模組/base
  - 使用者輸入的新增/更新要求應保存至 /sources/模組/requirements，保存內容依照原 /sources/updates 的設計，此兩者意圖重複，可以合併
  - 更新/輸入完成後的完整結果保存回 /.copilot/模組/base，完整中文版本翻譯結果保存至 /.copilot/模組/composed
  - changelog / README.md(原 TOOLS.md) /sources 的記錄內容規則不變
  - 使用者要求 release 以後，複製更新結果至 /.github(不用加上模組目錄) 與 /templates (要有模組目錄)，並將 changelog 與 /sources 內的內容更新至發行版本
```
