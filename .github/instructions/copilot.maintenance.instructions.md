...
# Scope
These rules apply only to the Copilot maintenance toolchain itself - `copilot.maintenance.instructions.md`, `copilot.maintainer.agent.md`, `copilot.maintain.prompt.md` - and the related source, composed, and published forms under `ai/**`. They should not automatically govern arbitrary downstream project-local `.github` rules or third-party vendor AI artifacts.
