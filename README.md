...
npx @saintber/copilot-library doctor
npx @saintber/copilot-library list
npx @saintber/copilot-library remove --target . --module kb
npx @saintber/copilot-library remove --target . --module all
```
...
> 若是舊版安裝、`state.json` 尚未記錄 `installedFiles`，請先執行一次 `update`，再使用 `remove`，以確保解除安裝安全。
