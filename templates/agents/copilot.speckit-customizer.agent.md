...
- When a `.github/` artifact is updated, also write the full Traditional Chinese counterpart to the matching `ai/composed/zh-TW/` path in the same run.
- The `ai/composed/zh-TW/` file must be the complete full-document Traditional Chinese version of the corresponding `.github/` artifact, not only the newly added customization sections.
...
3. Compare `ai/composed/zh-TW/speckit-backup/` with the current target Speckit artifacts before deciding what to change.
4. Update only the necessary Speckit-specific prompt, agent, or instruction files.
5. If a publish-layer artifact under `.github/` changes, create or update the corresponding full Traditional Chinese file under `ai/composed/zh-TW/`.
...
- Do not leave `ai/composed/zh-TW/` outputs as mixed-language or partially translated documents; the entire document must be in Traditional Chinese.
