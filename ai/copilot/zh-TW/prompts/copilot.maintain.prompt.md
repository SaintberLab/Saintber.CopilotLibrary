...
- `draft_en_path`（選填）：Draft English 輸出路徑（預設 `ai/<module>/en/`；fallback `ai/en/`）
- `draft_zh_tw_path`（選填）：Draft Traditional Chinese 輸出路徑（預設 `ai/<module>/zh-TW/`；fallback `ai/zh-TW/`）
- `deploy`（選填）：僅在使用者明確宣告 Deploy 時設為 `true`
...
1.7. 解析 canonical artifact 路徑再編輯。對 `copilot-instructions.md`，只使用 `ai/copilot/en/instructions/copilot-instructions.md` 與 `ai/copilot/zh-TW/instructions/copilot-instructions.md`；若存在非 canonical 路徑，標記為 skipped，除非明確要求遷移。
...
7. Normalize 措辭與章節結構。
8. 驗證跨產物一致性。
9. 有寫入權限時直接更新 `ai/<module>/en/` 與 `ai/<module>/zh-TW/` Draft 檔案。
10. 只有在 `deploy=true` 被明確宣告時，才更新 `.github/`。
11. 以清楚分段的結果回傳。
...
14.7. Release 宣告時，將所有已發布產物由 `.github/` 同步至 `/templates/<module>/`；必要的 repository-level 治理檔可保留在 `/templates/` 根目錄。
...
- 未明確宣告 `deploy=true` 時，不得修改 `.github/`。
- `release=true` 且有指定版號時，更新 `package.json` `version`。
- 將 English 與 Traditional Chinese Draft 內容分別寫入 `draft_en_path` 與 `draft_zh_tw_path`。
