...
# 適用範圍
本規則僅適用於 Copilot 維護工具鏈本身 - `copilot.maintenance.instructions.md`、`copilot.maintainer.agent.md`、`copilot.maintain.prompt.md` - 與 `ai/**` 下相關的 draft、deploy、release 產物；不作為套件使用者自有 `.github` 規則或第三方 vendor AI 檔案的預設治理層。

# 穩定規則
- `ai/` 為 authoring 層，`.github/` 為 deploy target。
- Draft 產物必須採用模組優先、語言次之、類型第三層的目錄結構：`ai/<module>/en/[type]/`、`ai/<module>/zh-TW/[type]/`，需求歷程存放於 `ai/<module>/sources/requirements/`。
- `copilot-instructions.md` 雖為 VS Code 保留檔名，仍屬於 `copilot` module；其 canonical draft 路徑固定為：`ai/copilot/en/instructions/copilot-instructions.md` 與 `ai/copilot/zh-TW/instructions/copilot-instructions.md`。
- 除非使用者明確要求遷移或相容性處理，否則不得在 canonical `ai/copilot/en|zh-TW/` 路徑以外建立或維護同一份 `copilot-instructions.md` 的平行軌。
...
- Draft 的 English 與 Traditional Chinese 版本必須保持同步：English 位於 `ai/<module>/en/[type]/`，Traditional Chinese 位於 `ai/<module>/zh-TW/[type]/`。
- 每次更新都必須產出清楚分段的結果。
...
- 一般維護（非 release）只更新 `ai/` draft 產物；除非明確宣告，否則不得更新 `.github/` 或 `/templates/`。
- Deploy 是明確階段。只有當目前 prompt 明確宣告 `deploy=true` 時，才可更新 `.github/`。
...
- 若目標是 Draft，需遵循 `ai/README.md` 定義的流程：`ai/<module>/en` 與 `ai/<module>/zh-TW` -> Deploy -> Release。
- Draft 更新不得寫入 `.github/`，除非在同一次 prompt 中明確宣告 Deploy。
- 若已宣告 Deploy 並更新 `.github/`，必須確保對應 English 與 Traditional Chinese draft 檔案已先同步更新。
